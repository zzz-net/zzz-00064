...
function getIsoDate(offsetDays) {
  const d = new Date('2090-01-01T00:00:00.000Z');
  d.setDate(d.getDate() + parseInt(RUN_ID.slice(-4)) + offsetDays);
...
async function testBug1_OverlapConfirm() {
  ...
  try {
    ...
    const DAY1 = getIsoDate(0);
    console.log(`1. 创建两张重叠时段的工单（隔离日期 ${DAY1}，运行标识 ${RUN_ID}）`);
    const { data: orderA } = await apiRequest('/orders', {
      method: 'POST',
      body: JSON.stringify({
        customerName: `BUG1-A-${RUN_ID}`,
        serviceType: '空调维修',
    ...
    await apiRequest(`/orders/${orderAId}/force-assign`, {
      method: 'PUT',
      body: JSON.stringify({ technicianId: 1, reason: `Bug1 测试-${RUN_ID}` }),
    });
...
async function testBug2_ForceAssignApproval() {
  ...
  try {
    ...
    const DAY2 = getIsoDate(30);
    console.log(`\n2. 创建一张新工单（隔离日期 ${DAY2}）`);
    const { data: order } = await apiRequest('/orders', {
      method: 'POST',
      body: JSON.stringify({
        customerName: `BUG2-${RUN_ID}`,
        serviceType: '家电维修',
        description: 'Bug2 测试',
    ...
    const { data: request } = await apiRequest(`/orders/${orderId}/force-assign-request`, {
      method: 'POST',
      body: JSON.stringify({ technicianId: 2, reason: `紧急工单 Bug2测试-${RUN_ID}` }),
    });
    ...
    const { data: approveResult } = await apiRequest(`/approvals/${approvalId}/approve`, {
      method: 'PUT',
      body: JSON.stringify({ remark: `同意 Bug2测试-${RUN_ID}` }),
    });
    ...
    const { data: start } = confirm.success
      ? await apiRequest(`/orders/${orderId}/start`, { method: 'PUT' })
      : { data: {} };
    if (confirm.success) {
      console.log(`   开始服务: ${start.success ? '成功 - ' + start.data.status : '失败 - ' + start.error}`);
    }
    const { data: complete } = start.success
      ? await apiRequest(`/orders/${orderId}/complete`, { method: 'PUT', body: JSON.stringify({ result: '修复完成 Bug2测试' }) })
      : { data: {} };
    if (start.success) {
      console.log(`   完成服务: ${complete.success ? '成功 - ' + complete.data.status : '失败 - ' + complete.error}`);
    }
    if (!complete.success) {
...
async function testAssignOverlapStillWorks() {
  ...
  try {
    ...
    const DAY3 = getIsoDate(60);
    console.log(`1. 创建两张重叠工单（隔离日期 ${DAY3}）`);
    const { data: orderA } = await apiRequest('/orders', {
      method: 'POST',
      body: JSON.stringify({
        customerName: `ASSIGN-A-${RUN_ID}`,
        serviceType: '空调维修',
        description: '分配重叠测试',
