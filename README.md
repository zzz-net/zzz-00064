...
```bash
node test-after-sale.mjs
```

测试通过数预期 86/86，共 9 组用例，覆盖：
- **权限边界**：客服/主管/管理员角色的访问控制（403 拦截）
- **回访登记**：按完成工单创建、满意度枚举校验、结果提交、重复保护
- **申诉全流程**：客服提交 → 主管受理 → 驳回/转派/标记解决，含状态流转校验
- **撤回冲突**：待受理可撤回、已处理不可撤回、撤回后再次申诉
- **CSV 导入**：缺列/无效工单/枚举值/失效行校验，原子性保证
- **CSV 导出**：回访按状态筛选、申诉表头与 UTF-8 BOM 兼容
- **操作日志**：配置、模板、申诉、撤回、导入全量记录

运行结束后会输出：`测试数据标记 UNIQUE_TOKEN=XXXXXXXX`（记下用于持久化验证）

### 7. 售后回访模块 跨重启持久化验证

```bash
# 步骤1：先跑售后回访回归测试，记下 UNIQUE_TOKEN
node test-after-sale.mjs
# 输出：测试数据标记 UNIQUE_TOKEN=XXXXXXXX

# 步骤2：重启后端服务

# 步骤3：运行持久化验证，传入 UNIQUE_TOKEN
node test-after-sale-persistence.mjs <UNIQUE_TOKEN>
```

持久化验证 42 项全部通过，覆盖：回访记录+结果、申诉状态+处理人+转派、所有操作日志、CSV 导出接口重启后仍可用。

### 8. 运行售后知识库与处理预案模块完整回归测试

```bash
node test-knowledge.mjs
```

测试通过数预期 103/103，共 9 组用例，覆盖：
- **权限边界**：客服不能审批/停用/回滚/导入（403 拦截），主管不能创建分类/导入
- **分类与配置**：增删改查、启用/停用、重名拦截、匹配阈值更新
- **条目全流程**：草稿创建 → 提交审核 → 审核通过发布 → 编辑生成 v2 → 审核 v2 → 停用 → 回滚 v1 生成 v3
- **驳回冲突测试**：驳回必填备注、驳回后回到草稿、草稿不可二次驳回/停用
- **匹配与命中**：分类加权+标签词+标题词+描述词综合打分、采用标记、效果三档反馈
- **CSV 导入校验**：缺列/非法分类/重复标题/无效时间 4 类错误拦截
- **导入原子性**：有效行成功+无效行拒绝，最终只新增 N 条不写半截数据
- **CSV 导出内容**：条目导出 + 命中记录导出，UTF-8 BOM + 正确表头
- **操作日志**：14 种 operation_type 全记录（创建/审核/发布/停用/回滚/命中/导入…）
...
# 步骤3：运行持久化验证，传入 UNIQUE_TOKEN
node test-knowledge-persistence.mjs <UNIQUE_TOKEN>
```

持久化验证 46 项全部通过，覆盖：
- **分类与配置**：启用状态、匹配阈值值 55、默认条目数
- **条目状态与版本**：published 条目 version>=3（回滚链）、驳回条目 status=draft+驳回备注保留、v1/v2/v3 三个版本历史 + can_rollback=true
- **命中记录**：采用状态=1、反馈 effectiveness=helpful、反馈备注含测试标记、条目命中计数保留
- **操作日志**：52 条操作记录、7 种关键类型全保留、含测试标记日志≥33 条
- **CSV 导出接口**：重启后条目导出 + 命中记录导出 Content-Type/BOM/表头完全正常

### 10. 运行冲突处理中心完整回归测试（含撤回+CSV导出）
